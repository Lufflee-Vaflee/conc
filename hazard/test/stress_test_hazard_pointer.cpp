...
TEST_F(HazardPointerStressTest, HighContentionDefaultDomain) {
    ...
    // Thread-safe storage for dynamically created nodes
    std::mutex node_pool_mutex;
    std::vector<std::unique_ptr<StressTestNode>> dynamic_nodes;
...
TEST_F(HazardPointerStressTest, MemoryPressureTest) {
    ...
    std::mutex node_creation_mutex;
    std::vector<std::unique_ptr<StressTestNode>> thread_local_nodes;
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            std::vector<std::unique_ptr<StressTestNode>> local_nodes;
            ...
            for (int i = 0; i < num_iterations; ++i) {
                ...
                if (i % retire_frequency == 0) {
                    ...
                    local_nodes.push_back(std::move(new_node));
                }
                
                // Force cleanup trigger by creating many retirements
                if (i % 100 == 0) {
                    for (int j = 0; j < 5; ++j) {
                        auto temp_node = std::make_unique<StressTestNode>(j);
                        hazard_pointer<StressTestNode>::retire(temp_node.release());
                        nodes_retired.fetch_add(1);
                    }
                }
            }
            
            // Move local nodes to shared storage
            {
                std::lock_guard<std::mutex> lock(node_creation_mutex);
    ...
    std::cout << "Memory pressure test results:\n"
              ...
              << "  Thread local nodes kept alive: " << thread_local_nodes.size() << "\n";
}
